# PROJECT_ANALYSIS — PIANA (Roberto Piana studio site)

> Canonical project reference, as-built (June 2026). Verified against source.
> Quick dev start (commands) lives in [`README.md`](README.md) — this doc owns the
> architecture, content model, and decisions; it does not repeat the command list.

**I want to… →**
- run/edit locally → [`README.md`](README.md) · **§8 Tooling**
- understand how a page is built → **§2 Mental model**
- find where content lives / edit copy → **§4 Content model & CMS**
- change colours/fonts → **§6 Design system**
- understand the intro animation / JS → **§7 Front-end JS**
- deploy / CMS login → **§9 Hosting & CMS auth**
- know what's still fake/missing → **§11 Caveats** · **§12 Status**
...
(git-based), and hosted on **Netlify**. Minimalist, photography-led, with a hand-drawn
"Roberto Piana" **signature intro** (GSAP) and smooth scroll (Lenis). It descends from a
StyleShout template but only the flexbox grid in `base.css` remains (see §11).
...
## 2. Mental model — how a page is built
...
The raw client delivery bundle (`assets/` at repo root), `docs/`, and this file are **gitignored**.
...
`make` lists all targets (full list in [`README.md`](README.md)). Key flow:
...
- **Netlify** builds on push: `netlify.toml` → `command = "npm run build"`, `publish = "dist"`,
  `NODE_VERSION = 20`, long-cache headers for `/assets/*`, plus the language-404 `[[redirects]]`
  equivalents are served from `dist/_redirects`.
- **CMS auth** = Netlify **Identity** (invite-only) + **Git Gateway** (enable both in the Netlify
  ...
  Decap was chosen (see `docs/superpowers/specs`).
...
## 10. Accessibility (EN 301 549 / WCAG 2.1 AA)
